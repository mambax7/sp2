 ...
 * @return array
 */
function showSoapbox8Sbvotedata($options)
{
    require_once dirname(__DIR__) . '/class/sbvotedata.php';
    $moduleDirName = basename(dirname(__DIR__));
    $myts          = MyTextSanitizer::getInstance();

    $block         = array();
    $type_block    = $options[0];
    $nb_sbvotedata = $options[1];
    ...
    array_shift($options);
    if ($type_block) {
        $criteria->add(new Criteria('ratingid', 0, '!='));
        $criteria->setSort('ratingid');
        $criteria->setOrder('ASC');
    }

    $criteria->setLimit($nb_sbvotedata);
 ...
 * @return string
 */
function editSoapbox8Sbvotedata($options)
{
    require_once dirname(__DIR__) . '/class/sbvotedata.php';
    $moduleDirName = basename(dirname(__DIR__));
    ...
    /** @var XoopsObjectHandler $'. sbvotedata . 'Handler */
    $sbvotedataHandler = xoops_getModuleHandler('sbvotedata', $moduleDirName);
    $criteria          = new CriteriaCompo();
    array_shift($options);
    array_shift($options);
    ...
    foreach (array_keys($sbvotedataArray) as $i) {
        $ratingid = $sbvotedataArray[$i]->getVar('ratingid');
        $form     .= "<option value='" . $ratingid . "' " . (in_array($ratingid, $options) === false ? '' : "selected='selected'") . '>' . $sbvotedataArray[$i]->getVar('ratingid') . '</option>';
    }
    $form .= '</select>';
